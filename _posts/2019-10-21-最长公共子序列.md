...
## 什么是子序列
在数学中，某个序列的**子序列**是从最初序列通过去除某些元素但不破坏余下元素的相对位置（在前或在后）而形成的新序列。详情请查看[wike百科]([https://zh.wikipedia.org/wiki/%E5%AD%90%E5%BA%8F%E5%88%97](https://zh.wikipedia.org/wiki/%E5%AD%90%E5%BA%8F%E5%88%97)
)。
...
$L(i,j)
\left\{\begin{matrix}
L(i-1,j-1)+1,x_i=y_j,i\geqslant 1,j\geqslant 1
\\ 
max\begin{Bmatrix}  L(i,j-1),L(i-1,j)
\end{Bmatrix},x_i \neq x_y,i\geqslant 1,j\geqslant 1
\end{matrix}\right.
$
+ 解释
